...
int countS = 0; //for indexing sems
//int thrun = 0; 
...
void pthread_exit(void *value_ptr)
{
  ...
  if(mythreads[mythreads[count].join_id].basic_state == BLOCKED && mythreads[count].join_id != -1)
    {
      mythreads[mythreads[count].join_id].basic_state = mythreads[mythreads[count].join_id].saved_state;
    }
  schedule();
  unlock();
...
int pthread_join(pthread_t thread, void **value_ptr)
{
  ...
  while(mythreads[curr].id != thread && curr < MAX_THREADS)
    {
      curr++;
    }

  if(mythreads[curr].basic_state != EXITED)
    {
      mythreads[count].saved_state = mythreads[count].basic_state;
      mythreads[count].basic_state = BLOCKED;
      int i;
      for(i = 0; i < MAX_THREADS; i++)
	{
	  if(mythreads[curr].join_id == 0)
	    {
	      mythreads[curr].join_id = count;
	      break;
	    }
	}
      schedule();
  ...
  value_ptr = &mythreads[curr].exit_state;
...
{
  int curr;
  for(curr = 0; curr < MAX_THREADS; curr++)
    {
  ...
  mysems[curr].q = malloc(sizeof(struct queue));

  mysems[curr].q -> head = -10;
  mysems[curr].q -> tail = -10;
...
int sem_wait(sem_t *sem)
{
  ...
  while(mysems[curr].val == 0 && count != countS)
    {
      if(mysems[curr].q -> head < 0 && mysems[curr].q -> tail < 0)
	{
	  mysems[curr].q -> head = 0;
	  mysems[curr].q -> tail = 0;
	  mysems[curr].q -> array[mysems[curr].q -> tail] = mythreads[count].id;
	  mysems[curr].countB++;
      
	}
      else if(mysems[curr].q -> array[mysems[curr].q -> tail] != mythreads[count].id)
	{
	  mysems[curr].q -> tail++;
	  mysems[curr].q -> array[mysems[curr].q -> tail] = mythreads[count].id;
	  mysems[curr].countB++;
      ...
      if(mysems[curr].countB != 0)
	{
	  mysems[curr].q -> head = countS;
	  mysems[curr].q -> head++;
	  mysems[curr].countB++;
	  lock();
	  return 0;
...
{
  int curr = sem->__align;
  if(mysems[curr].val > 0 && mysems[curr].id == curr)
    {
      mysems[curr].val++;
    }
  unlock();
...
{
  int curr = sem->__align;
  if(mysems[curr].id == curr && mysems[curr].flag == 1 && mysems[curr].countB == 0)
    {
      mysems[curr].id = 0;
      mysems[curr].val = 0;
      mysems[curr].q = NULL;
